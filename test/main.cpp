...
int main(int argc, char* argv[])
{
    ...
    std::cout << "=== FLAC Codec Test ===" << std::endl;

    // Test parameters
    const std::string test_wav = "test_input.wav";
    const std::string test_flac = "test_output.flac";
    ...
    try
    {
        ...
        // Step 2: Create FLAC codec and test data
        {
            std::cout << "\n2. Creating FLAC codec and test audio data..." << std::endl;
        ...
        // Step 3: Encode to FLAC
        {
            std::cout << "\n3. Encoding to FLAC..." << std::endl;
            ...
            SoundFormat format{};
            format.SetAll(44100, 2, 16, total_samples, 2 * sizeof(AmInt32), eAudioSampleFormat_Float32);
            encoder->SetFormat(format);

            auto output_file = std::make_shared<DiskFile>(test_flac, eFileOpenMode_Write);
            ...
            encoder->Close();
            std::cout << "Successfully encoded " << offset << " samples to FLAC" << std::endl;

            // Check if FLAC file was created
            if (!std::filesystem::exists(test_flac))
            {
                std::cerr << "FLAC file was not created" << std::endl;
                return 1;
            }

            auto flac_size = std::filesystem::file_size(test_flac);
            auto wav_size = std::filesystem::file_size(test_wav);
            std::cout << "  FLAC file size: " << flac_size << " bytes" << std::endl;
            std::cout << "  Original WAV size: " << wav_size << " bytes" << std::endl;
            std::cout << "  Compression ratio: " << (static_cast<double>(flac_size) / wav_size * 100.0) << "%" << std::endl;
        }

        // Step 4: Decode from FLAC
        {
            std::cout << "\n4. Decoding from FLAC..." << std::endl;
            ...
            auto input_file = std::make_shared<DiskFile>(test_flac, eFileOpenMode_Read);
            ...
            std::cout << "Successfully decoded " << decoded_samples << " samples from FLAC" << std::endl;
        }

        // Step 5: Compare original and decoded audio
        {
            std::cout << "\n5. Comparing original and decoded audio..." << std::endl;
            audio_match_encoded = TestAudioBuffer::IsEqual(input_buffer, output_buffer);
            ...
            else
            {
                ...
                // This might be acceptable for lossy compression, but FLAC should be lossless
            }
        }
        ...
        // Step 7: Compare original and decoded audio
        {
            ...
            audio_match_decoded = TestAudioBuffer::IsEqual(input_buffer, output_buffer);
        ...
        // Step 8: Test results summary
        {
            ...
            std::cout << "[OK] FLAC encoding: SUCCESS" << std::endl;
            std::cout << "[OK] FLAC decoding: SUCCESS" << std::endl;
            ...
            std::cout << "  - " << test_flac << " (encoded FLAC)" << std::endl;
